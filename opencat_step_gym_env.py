...
class OpenCatStepGymEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = gym.spaces.Box(
            np.array([-1] * SIZE_OBSERVATION), np.array([1] * SIZE_OBSERVATION)
        )

    def step(self, action):
        p.configureDebugVisualizer(p.COV_ENABLE_SINGLE_STEP_RENDERING)
        last_position = p.getBasePositionAndOrientation(self.robot_id)[0][0]
        joint_angs = np.asarray(
            p.getJointStates(self.robot_id, self.joint_id), dtype=object
        )[:, 0]
        ...
        for idx in paw_idx:
            ...
            paw_clearance += (paw_z_pos - PAW_Z_TARGET) ** 2 * np.linalg.norm(
                (
                    p.getLinkState(self.robot_id, linkIndex=idx, computeLinkVelocity=1)[
                        0
                    ][0:1]
                )
            ) ** 0.5
        ...
        if self.step_counter % 2 == 0:
            self.angle_history = np.append(
                self.angle_history, self.randomize(joint_angs, RANDOM_JOINT_ANGS)
            )
        ...
        smooth_movement = np.sum(
            ...
            + FAC_SMOOTH_2
            * np.abs(joint_angs - 2 * joint_angs_prev + joint_angs_prev_prev) ** 2
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        joint_id = p.createConstraint(
            parentBodyUniqueId=step_id,
            parentLinkIndex=-1,  # the base of the step object
            childBodyUniqueId=plane_id,
            childLinkIndex=-1,  # the base of the plane
